...
| ✅ | 4 | `workspace_confinement_declaration_template` | StringTemplate | `_declaration_template` | body | `"Your workspace is {{workspace_path}}. Nothing outside this path exists..."` |
| ✅ | 5 | `output_tool_exclusivity_declaration_template` | StringTemplate | `_declaration_template` | body | `"{{tool_name}} is your only output mechanism. All output goes through this tool."` |
| ✅ | 6 | `batch_discipline_declaration_template` | StringTemplate | `_declaration_template` | body | `"Process in batches of {{batch_size}}. After every {{batch_size}} records..."` |
...
| ✅ | 2 | `rule_count_awareness_preamble_template_visible` | VisibilityMode | `"auto"` | → content #3 (threshold-gated) |
| ✅ | 3 | `rule_count_awareness_preamble_template_auto_threshold` | Integer | `5` | threshold for above |
| ✅ | 4 | `workspace_confinement_declaration_template_visible` | Boolean | `true` | → content #4 |
...
| ✅ | 8 | `output_tool_exclusivity_declaration_template_visible` | Boolean | `true` | → content #5 |
| ✅ | 9 | `batch_discipline_declaration_template_visible` | Boolean | `true` | → content #6 (also requires batch_size) |
...
PREAMBLE:
  ...
  ✅ rule_count_awareness_preamble_template "There are {{rule_count}} inviolable rules below..."
                                             [visible: rule_count_awareness_preamble_template_visible = "auto", threshold = 5]

BODY (rules, in visibility order):
  .workspace_path                          → GATE drives workspace_confinement_declaration_template rule
  .has_output_tool                         → GATE drives output_tool_exclusivity_declaration_template and batch_discipline_declaration_template
  .tool_name                               → interpolated into output_tool_exclusivity_declaration_template and batch_discipline_declaration_template
  .batch_size                              → interpolated into batch_discipline_declaration_template
                                             NOTE: batch_size is RootModel[int] — unwrap doesn't handle int

  ✅ workspace_confinement_declaration_template   "Your workspace is {{workspace_path}}..."
  ✅ output_tool_exclusivity_declaration_template "{{tool_name}} is your only output mechanism..."
  ✅ batch_discipline_declaration_template        "Process in batches of {{batch_size}}..."
...
`batch_size` is typed `OutputToolBatchSize` which is `RootModel[int]`. The current data unwrap only handles `RootModel[str]`. Interpolating `{{batch_size}}` into `batch_discipline_declaration_template` will fail or produce a wrong value.
