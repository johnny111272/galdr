...
| ✅ | 2 | `output_description_declaration_template` | StringTemplate | `_declaration_template` | body | `"You produce: {{DESCRIPTION}}"` |
| ✅ | 3 | `format_declaration_template` | StringTemplate | `_declaration_template` | body | `"Output format: {{FORMAT}}"` |
...
| ✅ | 6 | `directory_location_declaration_variant_template` | BaseModel | `_declaration_variant_template` | body | `{standard: "All output files are written under: {{OUTPUT_DIRECTORY}}", with_boundary: "Your output location is {{OUTPUT_DIRECTORY}}..."}` |
...
BODY:
  [SCALAR] description                     → interpolated into output_description_declaration_template
  [GATE] format                            → interpolated into format_declaration_template
  [GATE] name_known                        → GATE: drives name-related content selection
  [GATE] schema_embed                      → GATE: controls schema embedding

  ✅ output_description_declaration_template  "You produce: {{DESCRIPTION}}"
                                             interpolates .description
  ✅ format_declaration_template           "Output format: {{FORMAT}}"
                                             interpolates .format
  ...
  [SCALAR] output_directory                → interpolated into directory_location_declaration_variant_template
  ✅ directory_location_declaration_variant_template  {selector: "standard" → "All output files are written under: {{OUTPUT_DIRECTORY}}"}
...
**Fix required:** Content templates needed. For `name_instruction` — a passthrough `_body` field or `_declaration_template`. For `name_template` and `output_file` — conditional content labels with appropriate templates.
...
### ⚠️ ISSUE 4: `directory_location_declaration_variant_template` needs conditional — only when `output_directory` present
